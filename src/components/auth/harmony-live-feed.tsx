 ...
 */
export function HarmonyLiveFeed({ audience = "customer" }: { audience?: FeedAudience }) {
  ...
  return (
    <section className="auth-feed auth-executive-panel" aria-labelledby="harmony-feed-title">
      ...
      <div className="auth-feed__story" aria-live="polite">
        <p className="auth-kicker" id="harmony-feed-title">
          <span className="auth-live-dot" aria-hidden="true" />
          {t("label")}
        </p>
        <h2>{item?.title}</h2>
        <p className="auth-feed__body">{item?.body}</p>
        <Link href={toneDestination[item?.tone ?? "status"]} className="auth-inline-link">
          {t("learnMore")}
