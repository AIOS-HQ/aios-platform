 ...
 * catalog + the company's install-state + a `CompanyProfileSignal` derived from
 * the Company Context Envelope, so the pure engines run over live data.
 * Config/knowledge only — never secrets.
 */
...
export async function loadStorefrontContext(
  ...
  const [catalog, envelope, installState] = await Promise.all([
    loadCatalog(),
  ...
  return { catalog, signal, installedIds };
}
