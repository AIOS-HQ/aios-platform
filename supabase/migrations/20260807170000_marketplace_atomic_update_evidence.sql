-- Marketplace Production M3B: atomic update state + success evidence persistence.
-- Applies only to governed update success path.

create or replace function public.marketplace_apply_update_with_evidence(
  p_company_id uuid,
  p_item_id uuid,
  p_rows jsonb,
  ...
  p_reason_code text default 'update_applied'
)
...
begin
  ...
  if p_company_id is null or p_item_id is null then
    raise exception 'missing_update_subject' using errcode = '22023';
  end if;
  ...
  if coalesce(jsonb_typeof(p_rows), '') <> 'array' or jsonb_array_length(p_rows) = 0 then
    raise exception 'missing_update_rows' using errcode = '22023';
  end if;
  ...
  if coalesce(p_policy_evidence #>> '{subject,action}', '') <> 'update' then
    raise exception 'policy_action_mismatch' using errcode = '22023';
  end if;

  if p_evidence_from_version is not null then
    ...
    if v_installed_version is distinct from p_evidence_from_version then
      raise exception 'update_transition_conflict' using errcode = '22023';
    end if;
  end if;
  ...
  loop
    if coalesce(v_row->>'company_id','')::uuid <> p_company_id then
      raise exception 'update_row_company_mismatch' using errcode = '22023';
    end if;
    if coalesce(v_row->>'user_id','')::uuid <> v_user_id then
      raise exception 'update_row_user_mismatch' using errcode = '22023';
    end if;
  ...
  v_evidence_key := concat_ws(':',
    ...
    coalesce(p_evidence_to_version, 'none'),
  ...
  v_payload := jsonb_build_object(
    ...
    'toVersion', p_evidence_to_version,
...
revoke all on function public.marketplace_apply_update_with_evidence(
  uuid,
  uuid,
  jsonb,
  jsonb,
...
grant execute on function public.marketplace_apply_update_with_evidence(
  uuid,
  uuid,
  jsonb,
  jsonb,
