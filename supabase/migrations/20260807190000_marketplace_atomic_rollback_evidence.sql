-- Marketplace Production M4B-1: atomic rollback state + success evidence persistence.
-- Applies only to governed rollback success path.
...
begin
  ...
  if p_company_id is null or p_item_id is null then
    raise exception 'missing_rollback_subject' using errcode = '22023';
  end if;
  ...
  if coalesce(p_policy_evidence #>> '{subject,action}', '') <> 'rollback' then
    raise exception 'policy_action_mismatch' using errcode = '22023';
  end if;

  if coalesce(p_to_version, '') = '' then
    raise exception 'missing_rollback_target' using errcode = '22023';
  end if;
  ...
  if v_installed_version is null then
    raise exception 'rollback_installation_not_found' using errcode = '22023';
  end if;

  if v_installed_version is distinct from p_evidence_from_version then
    raise exception 'rollback_transition_conflict' using errcode = '22023';
  end if;

  if p_to_version is distinct from p_evidence_to_version then
    raise exception 'rollback_target_mismatch' using errcode = '22023';
  end if;
  ...
  if not found then
    raise exception 'rollback_installation_scope_mismatch' using errcode = '22023';
  end if;
